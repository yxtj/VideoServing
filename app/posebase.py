...
def computeOKS_NtoM(gtsList, dtsList, sigmas = None):
    assert gtsList.ndim == 3 and gtsList.shape[-2:] == (17,3)
    assert dtsList.ndim == 3 and dtsList.shape[-2:] == (17,3)
    sigmas = np.array(__KPT_OKS_SIGMAS__ if sigmas is None else sigmas)
    assert sigmas.shape == (17,)
    n = gtsList.shape[0]
    m = dtsList.shape[0]
    oks = np.zeros((n,m))
    for i in range(n):
        oks[i,:] = computeOKS_1toN(gtsList[i], dtsList, sigmas)
    return oks        
